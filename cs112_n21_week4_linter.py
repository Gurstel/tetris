...
import math
import platform
_module = 'cs112_n21_week4_linter.py version 1.0'
...
def _formatError(header, file, line, fn, text, msg, expl):
    ...
    if (file):
        messages.append('  File:     "%s"' % file)
    if (line):
        messages.append('  Line:     %d' % line)
    if (fn):
        messages.append('  Function: %s' % fn)
    if (text):
        messages.append('  Code:     %s' % text.strip())
    messages.append('  Error:    %s' % msg)
    if (expl):
        messages.append('  Hint:     %s' % expl)
    message = '\n'.join(messages)
    return message
...
'''
    ...
    def buildSimpleAST(self, ast, textOnly):
        ...
        if (not isinstance(ast[1], list)):
            ...
            if (result == ''):
                result = None
            if ((not textOnly) and (result != None)):
                result = ast
            return result
        result = []
        for val in ast:
